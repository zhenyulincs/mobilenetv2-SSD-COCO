...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, use_batch_norm=True, onnx_compatible=False,cfg=None):
        ...
        if expand_ratio == 1:
            ...
            if use_batch_norm:
                ...
                self.conv = nn.Sequential(
                    # pw
                    nn.Conv2d(inp, hidden_dim1, 1, 1, 0, bias=False),
                    nn.BatchNorm2d(hidden_dim1),
                    ReLU(inplace=True),
                    # dw
                    nn.Conv2d(hidden_dim1, hidden_dim1, 3, stride, 1, groups=hidden_dim1, bias=False),
                    nn.BatchNorm2d(hidden_dim1),
                    ReLU(inplace=True),
                    # pw-linear
                    nn.Conv2d(hidden_dim1, oup, 1, 1, 0, bias=False),
                    nn.BatchNorm2d(oup),
                )
            else:
                self.conv = nn.Sequential(
            ...
            for i in range(n):
                ...
                if i == 0:
                    self.features.append(block(input_channel, output_channel, s,
                                               ...
                                               onnx_compatible=onnx_compatible,cfg=cfgElement))
                else:
