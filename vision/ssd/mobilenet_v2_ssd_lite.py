...
def create_mobilenetv2_ssd_lite(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False,cfg=None):
    base_net = MobileNetV2(width_mult=width_mult, use_batch_norm=use_batch_norm,
                           onnx_compatible=onnx_compatible,cfg=cfg).features
    ...
    if cfg is not None:
        print(len(cfg))
        cfg_length = len(cfg)
        cfg = cfg[cfg_length-4:]
        extras = ModuleList([
        InvertedResidual(1280, 512, stride=2, expand_ratio=0.2,cfg=cfg[0]),
        InvertedResidual(512, 256, stride=2, expand_ratio=0.25,cfg=cfg[1]),
        InvertedResidual(256, 256, stride=2, expand_ratio=0.5,cfg=cfg[2]),
        InvertedResidual(256, 64, stride=2, expand_ratio=0.25,cfg=cfg[3])
    ])
    else:
        extras = ModuleList([
